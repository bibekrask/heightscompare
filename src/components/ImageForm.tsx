...
// Utility function to protect against Web3 interference
const protectFromWeb3Interference = (callback: () => void, duration: number = 1000) => {
  try {
    const originalError = console.error;
    const originalWarn = console.warn;
    
    // Suppress Web3-related errors temporarily
    console.error = function(...args) {
      ...
      if (message.includes('ethereum') || 
        ...
        console.warn('Suppressed Web3 error during protected operation:', ...args);
        return;
      }
    ...
    // Restore original handlers after the specified duration
    setTimeout(() => {
    ...
    console.warn('Error in Web3 protection wrapper:', error);
  }
};
...
const ImageForm: React.FC<ImageFormProps> = ({ 
  ...
  useEffect(() => {
    if (editingItem) {
      ...
      setUploadedImageSrc(editingItem.src?.startsWith('data:') ? editingItem.src : null);
    } else {
      setFormData(defaultFormData);
      setUploadedImageSrc(null);
    }
  }, [editingItem, defaultFormData]); // Add defaultFormData to dependency array
  ...
  const handleImageSelect = (e: React.ChangeEvent<HTMLInputElement>) => {
    protectFromWeb3Interference(() => {
      ...
      reader.onload = (event) => {
        try {
          ...
          console.warn('Error processing uploaded image:', error);
        }
      };
    ...
    }, 2000); // Longer duration for file processing
  };
  ...
  const clearImage = () => {
    ...
    // If editing, we need to handle this carefully since ImageForm doesn't have default images
    // For now, we'll keep the current image but allow user to upload a new one
    // In a future enhancement, this could show a placeholder or require a new image
    if (editingItem && onUpdate) {
      // Don't clear the existing image immediately, just reset the form state
      // The user will need to upload and crop a new image to replace the current one
  ...
  const handleDelete = () => {
    ...
    const confirmDelete = confirm(`Are you sure you want to delete "${editingItem.name || `Image ${editingItem.id.slice(0, 3)}`}"?`);
    if (confirmDelete) {
  ...
  return (
    <div ref={formRef} className="space-y-3 md:space-y-5">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-2">
              ...
              <span className="text-sm font-medium text-blue-800 dark:text-blue-300">
                Editing: {editingItem.name || `Image ${editingItem.id.slice(0, 3)}`}
              </span>
            </div>
            <button
          ...
          {uploadedImageSrc && !showCropper ? (
            ...
            <button 
              onClick={() => {
                ...
                }, 1500); // Duration for camera access
              }}
      ...
      {/* Add Image Button - only show when not editing */}
      {!editingItem && (
        <button
